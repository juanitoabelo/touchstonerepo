...
export default class Index extends Component {
  ...
  onSearchInsurance = () => {
    ...
    axios.get('https://touchstone-api.abelocreative.com/touchstone-ajax/ajax.php', {
      params: {
        ...
        queryType: 'getInsuranceSearchList'
      }
    })
     .then(function ({ data }) {
      let error = false;
  ...
  sortCol = (type) => {
    let { data, toggleName, toggleID } = this.state;
    let toggling;
    data.sort((a, b) => {
      if (type === `Name`) {
        var nameA;
      ...
      if (toggleID) return a.InsuranceID - b.InsuranceID;
      return b.InsuranceID - a.InsuranceID;
    });
    switch(type) {
      case 'Name':
    ...
    if(data !== undefined && data.length > 0) {
      ...
      return (<div>
        <table className="table table-striped table-hover">
          <thead>
              <tr>
                <th scope="col" onClick={sortCol.bind(this, 'ID')}>ID</th>
                <th scope="col" onClick={sortCol.bind(this, 'Name')}>INSURANCE NAME</th>
              </tr>
          </thead>
          <tbody>
          {
  ...
  render() {
    const { onSearchInsurance, onChangeStatus, display, state } = this;
                        ...
                        <Row>
                          ...
                          <label htmlFor="SearchVal">Search For</label>
                            <Input fullWidth size="Medium" className="SearchVal">
                              <input type="text" placeholder="" id="SearchVal" name="SearchVal" value={searchFor} onChange ={onChangeStatus.bind(this)}/>
                            </Input>
                          </Col>
